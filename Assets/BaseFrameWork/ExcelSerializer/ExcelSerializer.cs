...
namespace ExcelSerializer {
    ...
    public class ExcelSerializer {
        ...
        private static void GenerateBytes() {
            ...
            foreach(var file in files) {
                ...
                for(int i = 0; i < dt.Tables.Count; i++) {
                    var table = dt.Tables[i];
                    var clz = System.Type.GetType(ns + "." + table.TableName);
                    var listClz = System.Type.GetType(ns + "." + table.TableName + "s");
                    var listObj = System.Activator.CreateInstance(listClz);
                    var listField = listClz.GetField("configs");
                    ...
                    for(int r = 3; r < table.Rows.Count; r++) {
                        ...
                        for(int c = 0; c < table.Columns.Count; c++) {
                            ...
                            //fiedlType: " + fieldType + ", fieldVal: " + fieldVal);
        ...
        private static void GenerateCS(string clsName, List<FieldData> fields) {
            ...
            writer.WriteLine("[System.Serializable]");
            writer.WriteLine("public class " + clsName + "s {");
            writer.WriteLine("    public List<" + clsName + ">" + " configs = new List<" + clsName + ">();");
            writer.WriteLine("}");
    ...
    public class ExcelPrefWindow : EditorWindow {
        ...
        private void OnEnable() {
            ...
            m_Namespace = EditorPrefs.GetString(NamespaceKey, "ClientConfigs");
            m_ScriptsFolder = EditorPrefs.GetString(ScriptsFolderKey, "Scripts/ClientConfigs");
        }
    }
}